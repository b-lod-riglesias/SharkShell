...
export class AuthController {
    ...
    async changePassword(
        ...
        try {
            ...
            const result = await this.db.query('SELECT id, password_hash FROM users WHERE id = $1', [currentUser?.id]);
            if (result.rows.length === 0) {
            ...
            await this.db.query('UPDATE users SET password_hash = $1 WHERE id = $2', [newPasswordHash, user.id]);
